...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // canonical sensor plane one unit away from the pinhole.
	...
	y -= 0.5;
//	printf("screen:%f %f %f\n", vFov, hFov, ar);
	double sensorHeight = 2 * tan(radians(v_fov()) / 2) * 1;	// distance is always 1
	double sensorWidth = sensorHeight * ar;
	Vector3D vec = Vector3D(x * sensorWidth, y * sensorHeight, -1);
  return Ray(pos, c2w * vec.unit());
}
